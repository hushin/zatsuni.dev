...
modDatetime: 2026-01-26T22:41:56.000+09:00
...
音声認識結果をローカルで動かしているLLMに渡して整形する処理も実装した。

gpt-oss-20bで試したとき：私のPCスペックだと数秒の入力でも数十秒かかるため、現在は使っていない。もっと軽量なモデルや強いスペックであれば実用的になりそう。

追記: LFM2.5-1.2B-JP を使ったところ数秒の入力も1,2秒で処理できた。時間は実用的。
